;;; calendar-chinese.el --- calendar for chinese
...
;;
;;                (add-to-path 'load-path "path/where/to/calendar-chinese"
;;                (require 'calendar-chinese)
;; ChangeLog
...
(defvar calendar-chinese-indent 0
  "Indent to display chinese calendar")

(defun calendar-my-move-hook ()
  (my-calender-set-mode-line))
(add-hook 'calendar-move-hook 'calendar-my-move-hook)

(defun calendar-my-init-windows-hook()
  (my-calender-set-mode-line))
(add-hook 'calendar-initial-window-hook 'calendar-my-init-windows-hook)
...
(defvar calendar-chinese-jiqi-name
  ["小寒" "大寒" "立春" "雨水" "惊蛰" "春分"
   "清明" "谷雨" "立夏" "小满" "芒种" "夏至"
   "小暑" "大暑" "立秋" "处暑" "白露" "秋分"
   "寒露" "霜降" "立冬" "小雪" "大雪" "冬至"]
...
(defvar calendar-chinese-current-year-jieqi-list nil)
...
;;定义中国星期的显示格式
(defun my-chinese-week-name (date)
...
;;定义中国日期的显示格式
(defun calendar-my-display-form (date)
  (let* ((weekname (my-chinese-week-name date))
...
(setq calendar-date-display-form '((calendar-my-display-form date)))
(setq diary-date-forms chinese-date-diary-pattern)

;;定义节气的识别函数
;;24节气的计算方法是，
;;从冬至开始，地球围绕太阳每转动15度的那一刻就是一个节日
(defun calendar-chinese-next-jieqi-date (d)
"Return the next Jieqi after or on the absolute date d,
the Return data form is absolute date"
   (solar-date-next-longitude d 15))

(defun calendar-chinese-create-jieqi-list (year)
" Create the year's jieqi ,store it in the variable
calendar-chinese-current-year-jieqi-list.
The format is (year (month day) (month day) ... (month day))
"
  (if (not (equal calendar-chinese-current-year-jieqi-list nil))
      (setq calendar-chinese-current-year-jieqi-list nil))
(let* (
       (start-date
  ...
  (setq calendar-chinese-current-year-jieqi-list (list year))
  (while (< i 24)
    (setq i (1+ i))
    (setq temp-date  (calendar-chinese-next-jieqi-date start-date))
    (setq jieqi-date
	  (calendar-gregorian-from-absolute
	   (floor (calendar-astro-to-absolute temp-date))))
    (setq calendar-chinese-current-year-jieqi-list
	  (cons (list (car jieqi-date) (car (cdr jieqi-date)))
	  calendar-chinese-current-year-jieqi-list))
    (setq start-date (1+ temp-date)))

  (setq calendar-chinese-current-year-jieqi-list
	(nreverse calendar-chinese-current-year-jieqi-list))
  ))

(defun calendar-chinese-jieqi-p (month day year)
"If the date is jieqi return t,else return nil"
 ...
 (if (or (equal calendar-chinese-current-year-jieqi-list nil)
         (not
	  (equal (car calendar-chinese-current-year-jieqi-list) year)))
     (calendar-chinese-create-jieqi-list year))
 (if (member (list month day) calendar-chinese-current-year-jieqi-list)
     t
     nil))

(defun calendar-chinese-jieqi-name (month day year)
"Get the chinese jieqi name if that day is  a jieqi, else return nil"
 (if (calendar-chinese-jieqi-p month day year)
  (let ((name-index (+ (* 2 (1- month)) (/ day 15) )))
       (aref calendar-chinese-jiqi-name name-index))))
...
;;将阳历日期换算成阴历,返回汉字日期名称
(defun calendar-chinese-day-name(date)
...
(defun calendar-chinese-year-name (date)
...
(defun calendar-chinese-month-name (date)
"return chinese month name string ,the date form is (month day year)"
...
(defun my-calender-set-mode-line ()
  (let* ((date (calendar-cursor-to-date))
         ...
         (s2 (format "农历%s年%s%s"
		     (calendar-chinese-year-name date)
		     (calendar-chinese-month-name date)
		     (calendar-chinese-day-name date)))
         (s3 (calendar-my-cursor-holidays))
         (x (list "" s1 s2 s3 ""))
...
(defun calendar-chinese-day-name-displayed(month day year)
  ...
  (let* ((jieqi-name (calendar-chinese-jieqi-name month day year))
	 (month-name )
	 (c-day (nth 3
		   (calendar-chinese-from-absolute
	...
	jieqi-name
      (if (= c-day 1)
	  (let* ((month-name
		(calendar-chinese-month-name (list month day year))))
	...
	(calendar-chinese-day-name (list month day year))))
    )
)
...
"
  ...
  (setq string
	(replace-all-matched-text string "%Y"
				  (calendar-chinese-year-name date)))
  ;;Replacs %y with chinese month name
  (setq string
	(replace-all-matched-text string "%m"
				  (calendar-chinese-month-name date)))
  ;;Replacs %y with chinese date name
  (setq string
	(replace-all-matched-text string "%d"
				  (calendar-chinese-day-name date)))
  ...
  (setq string
	(replace-all-matched-text
	 string "%o"
	 (mapconcat 'identity (calendar-check-holidays date) "; ")))
  (format-time-string  string time universal))
 )


(defun replace-all-matched-text (string rexp replace-text)
...
(defun calendar-chinese-shuxiang-name (year)
...
(defun calendar-my-cursor-holidays ()
...
characters on the line."
  (let* ((blank-days;; at start of month
   ...
   (insert
    (calendar-string-spread
     (list (format "  %d年%d月    农历%s年%s    属相:%s"
		   year
		   month
		   (calendar-chinese-year-name (list month 1 year))
		   (calendar-chinese-month-name (list month 1 year))
		   (calendar-chinese-shuxiang-name year))
	   ) ?  36))
   ...
   (loop for i from 1 to last do
         (insert (propertize 
                  (format "%2d%s" i
                          (calendar-chinese-day-name-displayed  month i year))
...
; at best have unpredictable results for such dates.
  ...
  (erase-buffer)
  ;;只产生一个月的月历
  (setq calendar-chinese-indent (max 0 (/ (- (frame-width) (* 7 8)) 2)))
  (calendar-generate-month month year calendar-chinese-indent)
)
    ...
    (goto-line (+ 3 ;;日期从第三行开始
		  (/ (+ day  -1
                        (mod
                         ...
                         7))
                     7)))
    (move-to-column (+ calendar-chinese-indent
		       (* 8 (mod
  ...
  (let* ((sexp-mark (regexp-quote diary-sexp-entry-symbol))
    ...
    (while (re-search-forward s-entry nil t)
      (if (char-equal (preceding-char) ?\()
          (setq diary-marking-entry-flag t)
        (setq diary-marking-entry-flag nil))
      (re-search-backward "(")
      (let ((sexp-start (point))
        ...
        (loop for date from first-date to last-date do
...
A value of 0 in any position is a wildcard."
  (save-excursion
    ...
    (let ((m displayed-month)
          (y displayed-year))
          (calendar-mark-month m y month day year color);;21.3用该函数
...
like (11 2 1977 \"shuxiang\" \"sexagesimal\")."
  ...
  (let* ((year (or year (calendar-extract-year
    ...
    (list cmonth
          ...
          year
          (calendar-chinese-shuxiang-name year)
          (concat (calendar-chinese-sexagesimal-name cyear)
                  "年"
...
(defun diary-lunar-date (cmonth cday &optional mark)
  "Specific Chinese lunar date diary entry.
...
(provide 'calendar-chinese)
;;; calendar-chinese.el ends here
